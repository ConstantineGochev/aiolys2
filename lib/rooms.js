...
let rooms = {}; // The Object that contains all the room instances
const songsdb = clients.songs;
const usersdb = clients.users;
const privateclient = clients.privateclient
const isString = utils.isString;
const isUsername = utils.isUsername;
const publicRooms = require("../public_rooms.json")
let primus;
let sparks;
 ...
 * Room constructor.
 */
function Room(roomname,roomID,isPrivate = false) {
  ...
  this.roomID = roomID;
  // this.trimmedroomname = roomname.replace(/\s/g, "");
...
Room.prototype.addUser = function(spark, loggedin) {
  ...
  primus.send('updateoverview', this.roomID, this.totusers);
  spark.send('ready', {
...
Room.prototype.gameOver = function() {
  ...
  primus.room(this.roomID).send('gameover', podium);
...
Room.prototype.initialize = function() {
  const room = this;
  console.log("this roomname = ",this.roomname.toString())
  Playlist.findOne({name: this.roomname.toString()}, function(err, playlist) {
    if(err) {
      throw err;
    }
    room.trackscount = playlist.listAdded.length - 1;
    room.sendLoadTrack();
  })
};
Room.prototype.initPrivate = function () {
  this.initialize()
}
...
Room.prototype.onChatMessage = function(msg, spark, to) {
  ...
  primus.room(this.roomID).send('chatmsg', msg, from);
};
...
Room.prototype.onMatch = function(spark, what) {
  ...
  primus.room(this.roomID).send('updateusers', usersData);
...
Room.prototype.onPair = function(spark, allinone) {
  ...
  primus.room(this.roomID).send('updateusers', this.usersData);
};
...
Room.prototype.join = function(spark, nickname) {
  ...
  usersdb.exists(['user:' + nickname], function(err, exists) {
    ...
    if (!spark.nickname) {
      ...
      spark.join(room.roomID, function() {
...
Room.prototype.removeUser = function(nickname) {
  ...
  // Broadcast the event
  primus.send('updateoverview', this.roomID, this.totusers);
  primus.room(this.roomID).send('userleft', nickname, usersData);
};
...
Room.prototype.sendLoadTrack = function () {
    ...
    const room = this;
    Playlist.findOne({name: this.roomname}, function(err, playlist) {
      if (err) {
        throw err
      }
      const id = playlist.listAdded[index].trackId;
      const song = playlist.listAdded[index]
      // if (~room.playedtracks.indexOf(id)) {
      //   return room.sendLoadTrack();
      // }
      room.playedtracks.push(id);

      room.artistName = song.artistName;
      room.artist = room.artistName.toLowerCase();
      room.trackName = song.trackName;
      room.title = room.trackName.toLowerCase();
      room.feat = /feat\. (.+?)[)\]]/.test(room.title) ? RegExp.$1 : null;
      room.previewUrl = song.previewUrl;
      room.artworkUrl = song.artworkUrl30;
      room.trackViewUrl = song.trackViewUrl;
      primus.room(room.roomID).send('loadtrack', room.previewUrl);
...
Room.prototype.sendPlayTrack = function() {
  this.status = Room.PLAYING;
  this.songcounter++;
  // console.log("SEND PLAY TRACK")
  // console.log("this.usersData ", this.usersData)
  primus.room(this.roomID).send('playtrack', {
...
Room.prototype.sendTrackInfo = function() {
  primus.room(this.roomID).send('trackinfo', {
...
module.exports.rooms = (function fillRooms() {
  var itemsProcessed = 0

  publicRooms.forEach(function(r) {
     rooms[r.roomID] = new Room(r.name, r.roomID)
      itemsProcessed++
    })
    if (itemsProcessed === publicRooms.length) {
      return rooms
    }

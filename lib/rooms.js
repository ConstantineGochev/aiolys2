...
module.exports.rooms = (async function fillRooms() {
  const playlists = await Playlist.find({}).exec()
  playlists.forEach(function(pl) {
    let trimmedname = pl.name.replace(/\s/g, "")
    if(pl.type === Room.PUBLIC) {
      rooms[trimmedname] = new Room(pl.name);
    } else {
      rooms[trimmedname] = new Room(pl.name, true)
    }
  })
  return rooms
})()
...
function Room(roomname, isPrivate = false) {
...
Room.prototype.addUser = function(spark, loggedin) {
  ...
  primus.send('updateoverview', this.trimmedroomname, this.totusers);
  spark.send('ready', {
...
Room.prototype.gameOver = function() {
  ...
  primus.room(this.trimmedroomname).send('gameover', podium);
...
Room.prototype.onChatMessage = function(msg, spark, to) {
  ...
  primus.room(this.trimmedroomname).send('chatmsg', msg, from);
};
...
Room.prototype.onMatch = function(spark, what) {
  ...
  primus.room(this.trimmedroomname).send('updateusers', usersData);
...
Room.prototype.onPair = function(spark, allinone) {
  ...
  primus.room(this.trimmedroomname).send('updateusers', this.usersData);
};
...
Room.prototype.join = function(spark, nickname) {
  ...
  usersdb.exists(['user:' + nickname], function(err, exists) {
    ...
    if (!spark.nickname) {
      ...
      spark.join(room.trimmedroomname, function() {
...
Room.prototype.removeUser = function(nickname) {
  ...
  // Broadcast the event
  primus.send('updateoverview', this.trimmedroomname, this.totusers);
  primus.room(this.trimmedroomname).send('userleft', nickname, usersData);
};
...
Room.prototype.sendLoadTrack = function () {
    ...
    Playlist.findOne({name: this.roomname}, function(err, playlist) {
      ...
      primus.room(room.trimmedroomname).send('loadtrack', room.previewUrl);
...
Room.prototype.sendPlayTrack = function() {
  ...
  primus.room(this.trimmedroomname).send('playtrack', {
...
Room.prototype.sendTrackInfo = function() {
  primus.room(this.trimmedroomname).send('trackinfo', {

...
Room.prototype.sendLoadTrack = async function () {
  
  let Name = this.nickname ; 
  let apiData , playList; 

  // const apiUser = await axios.get(`http://localhost:8138/playlist/getUser`);
  
  if (Name) {
    apiData = await axios.get(`http://localhost:8138/api/playlist/choosePlayList?userName=${Name}`);

    if (apiData.data.data.data[0]){

      playList=  apiData.data.data.data[0].playlistName;
    } else {
      Name = undefined ;
    }
  }
  ...
  if (Name && playList !== "Hits"){

  

    const apiData = await axios.get(`http://localhost:8138/api/playlist?name=${playList}`);
    const music = apiData.data.data.data[0].listAdded;
    ...
    ran = Math.floor((Math.random() * len) + 1) - 1;
      

    
      if (this.exists.indexOf(ran) === -1 ){
        bool = true ; 
        this.exists.push(ran); 
      } else if (this.exists.length>=len) {
        this.exists=[] ;
      } 
  }
    ...
    room.artistName = My.artistName;
    room.artist = My.artistName.toLowerCase();
    room.trackName = My.trackName;
    room.title = My.trackName.toLowerCase();
    room.feat = My.feat ? My.feat : null;
    room.previewUrl = My.previewUrl;
    room.artworkUrl = My.artworkUrl30;
    room.trackViewUrl = My.trackViewUrl;

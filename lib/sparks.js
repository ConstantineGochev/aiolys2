...
const msgMap = {};
async function joinWaitingRoom(room, spark2) {

  const rooms = await require("./rooms").rooms
  ...
  spark2.on("trystartingroom", function(data) {
    privateclient.hgetall(data.roomId, function(err, res) {
      if (res.creator === data.currUser) {
        rooms[res.playlist].initPrivate();
        spark2.room(data.roomId).write({method: "start", playlist: res.playlist})
      }
    })
  })
...
async function joinRoom(room, spark) {
